...
const char* Answer = "{\"username\":\"ChatBot\",\"message\":\"default!\"}";
// 网站的根目录

const char* doc_root = "/data/AI-Chat-Server/resources";
...
void http_conn::init()
{
    ...
    m_linger = false;       // 默认持不保链接  Connection : keep-alive保持连接
...
http_conn::HTTP_CODE http_conn::parse_content( char* text ) {
    if ( m_read_idx >= ( m_content_length + m_checked_idx ) )
    {
        if (!strcasecmp(m_url, "/login") || !strcasecmp(m_url, "/register")){
            ...
            m_username = u;
            m_passwd = p;
            std::cout <<  "username :" <<  m_username  << " password : " << m_passwd << std::endl;
            return GET_REQUEST;
        }
            ...
            std::cout << "text :" << text << std::endl;
            auto jsonObj = json::parse(text);
            std::string username,message,model;
            username = jsonObj["username"];  
            message = jsonObj["message"];
            model = jsonObj["modle"];
          
            if (message.size() > MAXQUESION_LEN) return NO_REQUEST;
            strcpy(m_message, message.c_str());
            strcpy(m_model, model.c_str());
            m_username = username;
            return GET_REQUEST; 
        } 
        // else if( !strcasecmp(m_url, "/login.html") ){
...
http_conn::HTTP_CODE http_conn::process_read() {
    ...
    while (((m_check_state == CHECK_STATE_CONTENT) && (line_status == LINE_OK))
        ...
        // 获取一行数据
        text = get_line();
        m_start_line = m_checked_idx; // 当前检查的index
        ...
        switch ( m_check_state ) {
            ...
            case CHECK_STATE_HEADER: { // 解析请求头
                ...
                if ( ret == BAD_REQUEST ) {
                    return BAD_REQUEST;
                } else if ( ret == FILE_REQUEST ) { // 头部可能含有信息 get 请求
                    return do_file_request();  // 解析 具体的请求信息
                }
                break;
            }
...
http_conn::HTTP_CODE http_conn::do_json_request()
{
    if ( strcasecmp(m_url, "/send-message") == 0 ){
        std::string question = m_message;
        std::string tempanswer = ChatBot::GetInstance()->getChatResponse(question,m_model);
        ...
        // 在这儿登陆
        json JsonResponse;
        bool flag  = MysqlMgr::GetInstance()->CheckUserPassword(m_username,m_passwd);
        if(flag) JsonResponse["success"] = 1;
        ...
        bool flag = MysqlMgr::GetInstance()->InsertUser(m_username, m_passwd);
        if(!flag) {
